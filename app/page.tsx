  ...
  return <main className={phase}>
    ...
    <section className="screen">
      ...
      <div className="visual carousel" aria-label={`Carrousel des exercices, exercice actuel : ${moves[idx][0]}`}>
        {[idx-1,idx,idx+1].filter(n=>n>=0&&n<12).map(n=><div key={n} className={`slide ${n===idx?'current':'adjacent'} ${n===4?'stepUp':''}`} style={{backgroundPosition:n===4?'center':moves[n][2]}}><span>{n===idx?'MAINTENANT':n<idx?'PRÉCÉDENT':'ENSUITE'}</span><strong>{moves[n][0]}</strong></div>)}
      </div>
