...
export default function Home(){
  ...
  const next=useCallback(()=>{if(lock.current)return;lock.current=true;
    if(phase==='work'){setPhase('rest');setSec(15);end.current=Date.now()+15000;warned.current=false;say('Souffle. Quinze secondes de récupération.',false);}
  ...
  const duration=phase==='rest'?15:30,pct=(sec/duration)*100;
  const title=phase==='rest'?'Récupération':phase==='done'?'Terminé !':moves[idx][0];
  return <main className={phase}>
    {!soundReady&&<div className="soundGate"><div><span>PRÊT POUR 9 MINUTES ?</span><h2>ACTIVE LE SON.<br/>ON S’OCCUPE DU RESTE.</h2><p>Un seul clic. Ensuite, la séance complète s’enchaîne toute seule.</p><button onClick={activate}>◉ &nbsp; ACTIVER LE SON ET DÉMARRER</button></div></div>}
    {soundReady&&<div className="musicPlayer"><iframe width="220" height="124" src="https://www.youtube.com/embed/3ArOBAt5Ml0?autoplay=1&start=16&loop=1&playlist=3ArOBAt5Ml0&controls=0&playsinline=1&rel=0" title="Dillon Francis, Skrillex — Bun Up the Dance" allow="autoplay; encrypted-media" referrerPolicy="strict-origin-when-cross-origin"/><span><i/> MUSIQUE EN COURS</span></div>}
    <header><div className="logo"><i/>NEUF<span>MIN</span></div><div className="rule"><b>30</b> SEC ON <i/> <b>15</b> SEC OFF</div><div className="count">{String(idx+1).padStart(2,'0')} <span>/ 12</span></div></header>
    <section className="screen">
      <div className="copy"><div className="kicker"><i/>{phase==='rest'?'RESPIRE · LE PROCHAIN ARRIVE':phase==='done'?'CIRCUIT TERMINÉ':'HIIT DÉBUTANT · À FOND'}</div><h1>{title}</h1><p>{phase==='rest'?(idx<11?`Ensuite : ${moves[idx+1][0]}`:'Dernière récupération, tiens bon.'):(phase==='done'?'9 minutes. 12 exercices. Tu peux être fier de toi.':moves[idx][1])}</p><div className="steps">{moves.map((m,i)=><button key={m[0]} aria-label={`Aller à ${m[0]}`} onClick={()=>jump(i)} className={i===idx?'active':i<idx?'past':''}>{i+1}</button>)}</div></div>
    ...
    <footer><b>9:00</b> DURÉE RÉELLE <span/> Prépare une chaise stable et un mur <em>Écoute ton corps — arrête-toi en cas de douleur.</em></footer>
  </main>
}
